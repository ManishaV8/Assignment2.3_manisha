...
#the solution and report the m<-replicate(10,rnorm(10),simplify="matrix")
View(m)
df<-as.data.frame(m)        
View(df)
vect = as.vector(m)
...
end.time.normal-start.time.normal # diff gives time taken during the process. for this, the function to be selected all and then run
...
end.time.vect-start.time.vect # diff gives time taken during the process. for this, the function to be selected all and then run system time differences.
